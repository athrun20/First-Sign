...
void main() {
  group('AppConfig Formspree normalize', () {
    ...
    test('keeps full https URL', () {
      expect(
        AppConfig.normalizeFormspreeEndpoint('https://formspree.io/f/xpwzgkqr'),
        'https://formspree.io/f/xpwzgkqr',
      );
    });
  ...
  group('QuoteSubmitService helpers', () {
    ...
    test('composeHomeownerConfirmation includes score and screening note', () {
      ...
      expect(body, contains(CostCopy.shortLabel));
      expect(body, contains(CostCopy.inlineNote));
      expect(body, contains(CostCopy.footnote));
      expect(body.toLowerCase(), contains('not a contractor bid'));
    });

    test(
      'composeInviteeLeadMessage includes lead fields and screening note',
      () {
        final lead = _lead();
        ...
        expect(body.toLowerCase(), contains('first sign'));
        expect(body.toLowerCase(), contains('screening'));
  ...
  group('QuoteSubmitService invitee fan-out', () {
    test(
      'notifies matching contractors after team post (best-effort)',
      () async {
        ...
        expect(
          result.contractorsMatched,
          2,
        ); // atlanta + no-email still matches
    ...
    test('no invitee posts when no contractors match', () async {
      ...
      expect(
        posts,
        containsAll(['contractor_quote', 'homeowner_confirmation']),
      );
